...
package com.jacky.XHHlauncher.main;
...
import com.jacky.XHHlauncher.detail.InputSourceCardPresenter;
import com.jacky.XHHlauncher.detail.MediaModel;
import com.jacky.XHHlauncher.weather.WeatherModel;
import com.jacky.catlauncher.R;
import com.jacky.XHHlauncher.app.AppCardPresenter;
import com.jacky.XHHlauncher.app.AppDataManage;
import com.jacky.XHHlauncher.app.AppModel;
import com.jacky.XHHlauncher.detail.InputSourceModel;
import com.jacky.XHHlauncher.detail.MediaDetailsActivity;
import com.jacky.XHHlauncher.function.FunctionCardPresenter;
import com.jacky.XHHlauncher.function.FunctionModel;
import com.jacky.XHHlauncher.weather.WeatherCardPresenter;
...
public class MainActivity extends Activity {
    ...
    private void addWeather() {
        String headerName=getResources().getString(R.string.app_header_app_weather);
        ArrayObjectAdapter listRowAdapter = new ArrayObjectAdapter(new WeatherCardPresenter());

        List<WeatherModel> weatherModel = WeatherModel.getWeatherModel();
        int cardCount = weatherModel.size();
        for (int i = 0; i < cardCount; i++) {
            listRowAdapter.add(weatherModel.get(i));
        }
        HeaderItem header = new HeaderItem(0,headerName);
        rowsAdapter.add(new ListRow(header,listRowAdapter));
    }

    private void addInputSource() {
        String headerName=getResources().getString(R.string.app_header_inputsource_name);
        ArrayObjectAdapter listRowAdapter = new ArrayObjectAdapter(new InputSourceCardPresenter());

        List<InputSourceModel> inputsourcemodels = InputSourceModel.getInputSourceModel();
        int cardCount = inputsourcemodels.size();
        for (int i = 0; i < cardCount; i++) {
            listRowAdapter.add(inputsourcemodels.get(i));
        }
        HeaderItem header = new HeaderItem(0,headerName);
        rowsAdapter.add(new ListRow(header,listRowAdapter));
    }
